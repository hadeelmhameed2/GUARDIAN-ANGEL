...
* **Green ($\le$ 7)**: Low risk — focus on personal empowerment.
* **Yellow (8–18)**: Warning signs — preparation mode.
* **Red ($\ge$ 19)**: High risk — emergency protocols activated.
...
* **Multimodal Attachments & Stealth Voice Trigger**: Supports text descriptions, photo attachments (camera/gallery), and stealth voice-activated emergency recordings ("עזרה" / "הצילו") via background `SpeechRecognition` and `MediaRecorder` running silently from the calculator disguise. Recorded audio drafts are managed securely in a dedicated Drafts view before saving to the journal or discarding.
